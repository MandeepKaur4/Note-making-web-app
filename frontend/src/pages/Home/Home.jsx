...
const onSearchNote= async(query)=>{
  try{
const res = await axios.get("http://localhost:3000/api/note/search", {params:{query},withCredentials:true,
} 
)
if(res.data.success===false){
  ...
  handleClearSearch={handleClearSearch}/>
    ...
    <div className="container mx-auto">
      {allNotes.length>0 ?(
        ...
        isSearch ?"https://tse4.mm.bing.net/th?id=OIP.kLpBEcgNkoAhUuQcemovoAHaD4&pid=Api&P=0&h=180"
        :
        "https://tse1.mm.bing.net/th?id=OIP.T_OhIDeRW97ikfw6sT4vdQHaE8&pid=Api&P=0&h=180"
      }
      message={isSearch ?"Oops!No notes found matching your search":
        `Ready to capture your ideas? click the'Add' button to start noting down your thoughts , inspiration and reminders. Lets get started`}/>
    )}
      
       
    </div>

    <button className='w-16 h-16 flex items-center  justify-center rounded-2xl bg-[#2B85FF] hover:bg-blue-600 absolute right-10 bottom-10'onClick={() =>{ 
